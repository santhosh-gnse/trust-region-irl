...
class DiscriminatorShapedSA(nn.Module):
    ...
    def __call__(self, x, a, x_n, absorbing, logp, shaping: float = 1.0):
        ...
        xa_n = jnp.concatenate([x_n.flatten(), a.flatten()]) # only used if absorbing is true
        ...
        reward = reward - shaping * logp # D = sigmoid(reward) and we compute loss using sigmoid BCE
