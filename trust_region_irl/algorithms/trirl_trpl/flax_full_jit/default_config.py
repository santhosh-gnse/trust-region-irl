...
def get_config(algorithm_name):
    ...
    config.total_timesteps = 30e6
    config.learning_rate = 0.000158
    ...
    config.epsilon = 0.6931393089159753
    ...
    config.gp_alpha = 0.428

    config.handle_absorbing_states = True
    config.reward_fn_approximator = True
    config.on_demand_etas = True
    config.reward_type = 'state-action' # options: state-action, state-based, shaped, shaped-sa
