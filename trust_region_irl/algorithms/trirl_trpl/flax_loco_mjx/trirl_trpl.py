...
from trust_region_irl.algorithms.trirl_trpl.flax_loco_mjx.policy import ActorCritic
from trust_region_irl.algorithms.trirl_trpl.flax_loco_mjx.discriminator import get_discriminator, get_reward_approximator
import os
...
import pickle
from trust_region_irl.algorithms.trirl_trpl.flax_loco_mjx.reward_correction import make_chunked_ensemble_rew_correct
from trust_region_irl.algorithms.trirl_trpl.flax_loco_mjx.trust_region_layer import *
...
class TRIRLSummaryMetrics(SummaryMetrics):
    ...
    eta_mu: float = 0.0
    eta_cov: float = 0.0
    kl_mean_part: float = 0.0
    kl_cov_part: float = 0.0
    post_proj_kl_mean_part: float = 0.0
    post_proj_kl_cov_part: float = 0.0
...
class TRIRLAgentConf(AgentConfBase):
    ...
    def from_dict(cls, d):
        ...
        tx, disc_tx, reward_approximator_tx = TRIRL_TRPL._get_optimizer(config)
...
class TRIRL_TRPL(JaxRLAlgorithmBase):
    ...
    def prepare_config_and_expert_dataset(self, config, env):
        ...
        if config.warm_start:
            assert config.reward_fn_approximator == True, "Cannot warm start without learning a reward approximator!"
    ...
    def load(config, env, run_path, writer, explicitly_set_algorithm_params):
        model = TRIRL_TRPL(config, env, run_path, writer, skip_init=True)
    ...
    def _train_fn(cls, rng, env, writer,
        ...
        etas_buffer = EtasBuffer.create(jnp.ones((config.num_steps, config.num_envs)), config.disc_buffer_capacity-1)
        policy_buffer = ParamsBuffer.create(train_state, config.disc_buffer_capacity) # only used if on_demand_etas == True
        reward_approximator_buffer = ParamsBuffer.create(reward_approximator_train_state, config.disc_buffer_capacity)
        if config.on_demand_etas:
            policy_buffer = ParamsBuffer.add(policy_buffer, train_state)

        # TRAIN LOOP
        def _update_step(runner_state, unused):
            # COLLECT TRAJECTORIES
            def _env_step(runner_state, unused):
                train_state, disc_train_state, reward_approximator_train_state, env_state, last_obs, train_state_buffer, disc_buffer, policy_buffer, reward_approximator_buffer, etas_buffer, rng = runner_state
                ...
                runner_state = (train_state, disc_train_state, reward_approximator_train_state, env_state, obsv, train_state_buffer, disc_buffer, policy_buffer, reward_approximator_buffer, etas_buffer, rng)
            ...
            chunked_correct = make_chunked_ensemble_rew_correct(
                ...
                maximum_eta= not config.on_demand_etas
            )
            ...
            if config.on_demand_etas:
                ...
                chunked_compute_etas = make_chunked_compute_etas(
                    policy_apply_fn=cls._vmap_plcy_apply_fn,
                    kl_cov_proj=kl_cov_proj,
                    nr_steps=config.num_steps, 
                    nr_envs=config.num_envs,
                    mean_bound=config.mean_bound,
                    cov_bound=config.cov_bound,
                )
                ...
                proj_etas = chunked_compute_etas(
                    ...
                    buffer_size=config.disc_buffer_capacity
                )
            else:
            ...
            if config.warm_start:
                init_corr_reward = jax.lax.cond(level < config.disc_buffer_capacity,
                            lambda _: jnp.zeros_like(traj_batch.reward),
                            lambda _: cls._get_reward_approximator_prediction((traj_batch.obs.reshape((-1, traj_batch.obs.shape[-1])),
                                                                    ...
                                                                    reward_approximator,
                                                                    ParamsBuffer.sample_oldest(reward_approximator_buffer)).reshape(traj_batch.reward.shape),
                            operand=None
                            )

            else:
                init_corr_reward = jnp.zeros_like(traj_batch.reward)
            ...
            if config.handle_absorbing_states:
                corr_reward_absorbing_state = chunked_correct(
                    ...
                    proj_etas,
                    jnp.zeros_like(traj_batch.reward), # if reward approximator is true, then this is recomputed. Else warm starting cannot work at all
                    level,
                    chunk_size=config.chunk_size,
                ).reshape(traj_batch.reward.shape)
            else:
            ...
            if config.reward_fn_approximator:
                ...
                # Add to buffer
                reward_approximator_buffer = ParamsBuffer.add(reward_approximator_buffer, reward_approximator_train_state)
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        action_std = jnp.exp(action_logstd) # (as_shape, )
                        old_action_mean = traj_batch.action_mean # (batch_size, as_shape)
                        old_action_logstd = traj_batch.action_logstd # (batch_size, as_shape)
                        old_action_logstd = old_action_logstd[-1] # (as_shape, )
                        old_action_std = jnp.exp(old_action_logstd) # (as_shape, )
                        ...
                        # ENTROPY PROJECTION
                        old_entropy = jnp.sum(old_action_logstd + 0.5 * jnp.log(2.0 * jnp.pi * jnp.e))
                        tau = 0.5
                        current_steps = jnp.max(traj_batch.metrics.timestep * config.num_envs)
                        beta = old_entropy * tau ** (10 * jnp.clip(current_steps, max=config.total_timesteps) / config.total_timesteps)
                        as_dim = env.mdp_info.action_space.shape[0]
                        # Entropy projection before KL projection
                        # action_logstd = entropy_projection(action_logstd, beta, dim=as_dim)

                        # KL DIVERGENCE PROJECTION
                        proj_action_mean, proj_action_std, eta_mu, eta_cov, kl_mean_part, post_proj_kl_mean_part, kl_cov_part, post_proj_kl_cov_part = kl_projection(action_mean, 
                                                                                                                                                                    action_std, 
                                                                                                                                                                    old_action_mean, 
                                                                                                                                                                    old_action_std, 
                                                                                                                                                                    config.mean_bound, 
                                                                                                                                                                    config.cov_bound)
                        max_eta = jnp.maximum(eta_mu, eta_cov)
                        # Entropy projection after KL projection
                        proj_action_logstd = jnp.log(proj_action_std)
                        proj_action_logstd = entropy_projection(proj_action_logstd, beta, dim=as_dim)
                        ...
                        tr_loss_maha = 0.5 * jnp.mean(jnp.sum(((proj_action_mean_det - action_mean)/ proj_action_std_det) ** 2, axis=1))
                        tr_loss_cov_part = 0.5 * jnp.mean(jnp.sum(2.0 * (jnp.log(proj_action_std_det) - jnp.log(action_std)) + (action_std/proj_action_std_det)**2 - 1.0))
                        trust_region_loss = tr_loss_maha + tr_loss_cov_part

                        pi = distrax.MultivariateNormalDiag(proj_action_mean, jnp.exp(proj_action_logstd))
                        ...
                        total_loss = (
                            ...
                            + config.trust_region_coef * trust_region_loss
                        )
                        return total_loss, (value_loss, loss_actor, trust_region_loss, entropy, max_eta, (eta_mu, eta_cov, kl_mean_part, post_proj_kl_mean_part, kl_cov_part, post_proj_kl_cov_part))
            ...
            max_etas = loss_info[1][4]
            max_etas = jnp.mean(max_etas, axis=0).reshape(config.num_steps, config.num_envs) # (num steps, num envs)
            etas_buffer = EtasBuffer.add(etas_buffer, max_etas)

            eta_mus, eta_covs, kl_mean_part, post_proj_kl_mean_part, kl_cov_part, post_proj_kl_cov_part = loss_info[1][5]
            ...
            metric = TRIRLSummaryMetrics(
                ...
                eta_mu = jnp.mean(eta_mus),
                eta_cov = jnp.mean(eta_covs),
                kl_mean_part = jnp.mean(kl_mean_part),
                kl_cov_part = jnp.mean(kl_cov_part),
                post_proj_kl_mean_part = jnp.mean(post_proj_kl_mean_part),
                post_proj_kl_cov_part = jnp.mean(post_proj_kl_cov_part)
            )
            ...
            def _evaluation_step():
                ...
                obsv, env_state = env.reset(reset_rng)
                runner_state_eval = (train_state, disc_train_state, reward_approximator_train_state, env_state, obsv, train_state_buffer, disc_buffer, policy_buffer, reward_approximator_buffer, etas_buffer, rng)
            ...
            def metrics_callback(metric):
                rlx_logger.info("┌" + "─" * 36 + "┬" + "─" * 16 + "┐")

                metric, logged_metrics, logging_corr_reward, loss_info, log_std, discrim_loss, approximator_loss, misc = metric
                total_policy_loss, (value_loss, loss_actor, loss_trust_region, entropy, _, _) = loss_info
                max_etas, proj_etas, level = misc
                ...
                log_dict = {
                    ...
                    ########################################
                    "projection/eta_mu": metric.eta_mu,
                    "projection/eta_cov": metric.eta_cov,
                    "projection/kl_mean_part": metric.kl_mean_part,
                    "projection/kl_cov_part": metric.kl_cov_part,
                    "projection/post_proj_kl_mean_part": metric.post_proj_kl_mean_part,
                    "projection/post_proj_kl_cov_part": metric.post_proj_kl_cov_part,
                    "projection/buffer_size": level,
                    ########################################
                ...
                if config.on_demand_etas:
                    log_dict["projection/mean_on_demand_eta"] = jnp.mean(proj_etas)
                else:
                    log_dict["projection/max_eta"] = jnp.max(max_etas)
                ...
                for name, value in log_dict.items():
                    ...
                    rlx_logger.info(f"│ {name.ljust(35)}│ {str(value).ljust(14)[:14]} │", flush=False)

                rlx_logger.info("└" + "─" * 36 + "┴" + "─" * 16 + "┘")

            jax.debug.callback(metrics_callback, (metric, logged_metrics, logging_corr_reward, loss_info, train_state.params["log_std"], discrim_loss_info, approximator_loss_info, (max_etas, proj_etas, level)))
            ...
            runner_state = (train_state, disc_train_state, reward_approximator_train_state, env_state, last_obs, train_state_buffer, disc_buffer, policy_buffer, reward_approximator_buffer, etas_buffer, rng)
            return runner_state, (metric, validation_metrics)

        rng, _rng = jax.random.split(rng)
        runner_state = (train_state, disc_train_state, reward_approximator_train_state, env_state, obsv, train_state_buffer, disc_buffer, policy_buffer, reward_approximator_buffer, etas_buffer, _rng)
        ...
        return {"agent_state": agent_state,
                "buffers": {"disc_buffer": runner_state[6], "policy_buffer": runner_state[7], "etas_buffer": runner_state[8]},
    ...
    @classmethod
    def _plcy_apply_fn(cls, obs, network, train_state):
        y, _ = network.apply({'params': train_state.params, 'run_stats': train_state.run_stats},
                                obs, mutable=["run_stats"])
        _, _, action_mean, action_logstd = y

        return action_mean, action_logstd


    @classmethod
    def _vmap_plcy_apply_fn(cls, obs, network, train_state):

        vmap_plcy_apply = jax.vmap(cls._plcy_apply_fn, in_axes=(0, None, None), out_axes=(0,0))
        action_mean, action_logstd = vmap_plcy_apply(obs, network, train_state)
