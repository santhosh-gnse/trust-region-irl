...
def make_chunked_compute_etas(policy_apply_fn, kl_cov_proj, nr_steps: int, nr_envs: int, mean_bound: float, cov_bound: float):
    ...
    vmapped_policy = jax.vmap(
        policy_apply_fn,
        in_axes=(None, None, 0),   # (states, network, params_batch)
        out_axes=(0, 0),
    )
    ...
    @partial(jax.jit, static_argnames=('network', 'chunk_size'))
    def process_chunk_on_device(
        etas, network, params_chunk, states,
        available: jnp.ndarray, chunk_size: int, write_index: jnp.ndarray
    ):

        mean, logstd = vmapped_policy(states, network, params_chunk)  # mean: (chunk_size, batch, A), logstd: (chunk_size, A)
    ...
    @partial(jax.jit, static_argnames=('network', 'chunk_size', 'buffer_size'))
    def chunked_compute_etas(policy_buffer, network, states, level: jnp.ndarray, chunk_size: int, buffer_size: int):
        """
...
kl_projection = jax.vmap(kl_projection, in_axes=(0, None, 0, None, None, None), out_axes=(0, None, 0, None, 0, 0, None, None))
