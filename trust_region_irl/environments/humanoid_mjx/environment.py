...
from trust_region_irl.environments.humanoid_mjx.state import State
from trust_region_irl.environments.humanoid_mjx.box_space import BoxSpace
from trust_region_irl.environments.humanoid_mjx.viewer import MujocoViewer
...
class Humanoid:
    ...
    def feature_from_transition(self, observation, action, eps=1e-4):
        ...
        def feature_from_transition_base(observation, action, eps=1e-4):
            ...
            cinert_dim = (self.mjx_model.nbody - 1) * 10
            cvel_dim = (self.mjx_model.nbody - 1) * 6
            actuator_force_dim = self.mjx_model.nv - 6

            contact_start = (qpos_without_xy_dim + qvel_dim + cinert_dim + cvel_dim + actuator_force_dim)

            contact_force = observation[:, contact_start:]
            torso_height = observation[:, 0]
            global_vel_x = observation[:, qpos_without_xy_dim + 0]

            z1 = global_vel_x
            z2 = torso_height
            z3 = jnp.sum(jnp.square(contact_force), axis=-1)
