...
from trust_region_irl.environments.point_maze_mjx.state import State
from trust_region_irl.environments.point_maze_mjx.box_space import BoxSpace
from trust_region_irl.environments.point_maze_mjx.viewer import MujocoViewer


class PointMaze:
    def __init__(self, render, horizon=100, reward_style="dense", success_radius=0.1, feature_fn="base"):
    ...
    def feature_from_transition(self, observation, action, eps=1e-4):
        def feature_from_transition_base(observation, action, eps=1e-4):
            ...
            particle_pos = observation[:, :2]
            target_pos = observation[:, 2:4]
            diff = particle_pos - target_pos
            reward_dist = -jnp.linalg.norm(diff, axis=-1)
            reward_ctrl = -jnp.sum(jnp.square(action), axis=-1)

            features = jnp.stack([reward_dist, reward_ctrl], axis=-1)
